...
import { Copy, Download, ExternalLink, Github, RefreshCcw } from "lucide-react";
...
export default function App() {
  ...
  const fetchImagesFromTab = useCallback(async () => {
    setLoading(true);
      ...
      const results = await chrome.scripting.executeScript({
        ...
        func: () => {
          ...
          imgElements.forEach((img, index) => {
            ...
            const sources = [
              img.src,
              img.currentSrc, // For <picture> elements
          ...
          // Extract from <picture> elements with <source> tags
          ...
          pictureElements.forEach((picture, picIndex) => {
            ...
            sources.forEach((source) => {
              ...
              if (srcset) {
                // Extract first URL from srcset (format: "url1 1x, url2 2x" or "url1 100w")
          ...
          // Extract from CSS background images (limited - only inline styles)
          ...
          elementsWithBg.forEach((el, index) => {
            ...
            if (style) {
              ...
              if (match && match[1]) {
                addImage(
                  match[1],
                  0, // Can't determine dimensions from CSS
                  0,
                  el.getAttribute("alt") ||
                    el.getAttribute("title") ||
                    el.getAttribute("aria-label") ||
                    "",
                  imgElements.length + pictureElements.length + index
                );
              }
            }
          });

          // Extract from <svg> elements with embedded images
          const svgElements = document.querySelectorAll("svg image");
          svgElements.forEach((svgImg, index) => {
            const href =
              svgImg.getAttribute("href") || svgImg.getAttribute("xlink:href");
            if (href) {
              const width = parseFloat(svgImg.getAttribute("width") || "0");
              const height = parseFloat(svgImg.getAttribute("height") || "0");
              addImage(
                href,
                width,
                height,
                svgImg.getAttribute("alt") || "",
                imgElements.length +
      ...
      <div className="flex justify-between items-center mb-4 sm:mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            onClick={() =>
              window.open(
                "https://github.com/lwshakib/segment-image-extractor",
                "_blank"
              )
            }
            variant="outline"
